...
pytest tests/ -m "not llm and not gpu"               # 172 tests, ~90 s, 99% coverage
...
## Layer 1 — deterministic (172 tests, 99% coverage)
...
## 🐞 The bug ledger — 17 real bugs, each pinned by a named test
...
| 17 | **voice eval leaked 40% of its sentences from training** — every eval speaker was also a training speaker; measured rather than assumed, and the unleaked subset scored *better* | 04 data | `test_voice_eval_leakage_is_measured_not_assumed` |
