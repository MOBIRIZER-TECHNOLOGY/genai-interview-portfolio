...
| **tests** | [pytest + DeepEval](tests/) · [INTERVIEW](tests/INTERVIEW.md) | **156 deterministic tests at 99% coverage** (regression tests mutation-verified against the real bugs) + judge-calibrated LLM quality gates | the DeepEval layer **found a real hallucination** that carried a valid citation and passed every mechanical check |
...
├── 07_rag_at_scale/
├── 08_rag_paradigms/
└── tests/               156 deterministic tests (99% coverage) + DeepEval gates
