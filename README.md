...
| **tests** | [pytest + DeepEval](tests/) · [INTERVIEW](tests/INTERVIEW.md) | **170 deterministic tests at 99% coverage** (regression tests mutation-verified against the real bugs) + judge-calibrated LLM quality gates | the DeepEval layer **found a real hallucination** that carried a valid citation and passed every mechanical check |
...
├── 07_rag_at_scale/
├── 08_rag_paradigms/
└── tests/               170 deterministic tests (99% coverage) + DeepEval gates
    ├── README.md        the two layers, judge calibration, the 15-bug ledger
...
**Fifteen real bugs found and pinned by tests across the whole repo** — not
...
[tests/README.md](tests/README.md#-the-bug-ledger--15-real-bugs-each-pinned-by-a-named-test) — including a chunker
