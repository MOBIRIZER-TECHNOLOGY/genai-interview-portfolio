...
> examples. 59 seconds on one consumer GPU, 34 MB adapter, 1.75% of parameters
...
weakness: `error_code` at 84.2% while everything else is ≥96.7%. Diagnosing it —
...
it needs a learned mapping. A 0.5B does it at 84% for 59 seconds of training and
runs anywhere.
...
800 synthetic examples, 120 held out, generated by `make_dataset.py`. Every
operator report is textually unique. The label space deliberately is not: 5
components, 3 severities, 10 error codes plus null, and **9 distinct actions
across all 800 rows**.

I would volunteer the problem with that, because it changes one of my own
headline numbers. **Four of the five components have exactly one action in the
whole dataset**, so `action` is nearly a lookup keyed on `component`. Measured:
a predictor that just copies each component's majority action scores **86.7%**
on the held-out set. My fine-tuned model scores 95.8%.

So `action` at 95.8% is the *least* impressive number in the project, not one of
the best — nine points over a lookup table. Compare that with `severity`, where
the majority-class baseline is 39.2% and the model reaches 100%: sixty points of
real learning. **A per-field accuracy is meaningless until you know what a stupid
predictor scores on the same field**, and that is a property of the dataset, not
the model.

It also explains a failure mode I found separately: a cosmetic font complaint
was assigned "restart ntp-relay in the cell namespace". The model never learned
remediation, it learned component → action, so a wrong component drags a
confidently wrong action with it.

The fix is dataset-side — more distinct actions per component, so the field
requires reading the report rather than classifying the service. `inspect_dataset.py`
prints the baselines next to the metrics, which is how I would want any eval
reported.
